#ifndef WORD_PROCESSING_UTILS_H
#define WORD_PROCESSING_UTILS_H
...
static void lookupWords ()
{
  ...
  for(;;) {
    ...
    std::cin >> lookupWord;
    if (std::cin.eof()) {
      // NOTE: EOF will behave differently depending on if ran with IDE or terminal
